            ...
            @else
                <form method="POST" action="{{ url('/penjualan/' . $penjualan->penjualan_id) }}" class="form-horizontal">
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-11">
                            <select class="form-control" id="penjualan_tanggal" name="penjualan_tanggal" required>
                                <option value="">- Pilih Tanggal -</option>
                                @foreach ($penjualan as $item)
                                    <option value="{{ $item->penjualan_id }}" @if ($item->penjualan_id == $stok->penjualan_id) selected @endif>
                                        {{ $item->nama }}
                                    </option>
                                @endforeach
                            </select>
